...
export class FilePreviewContentComponent {
  ...
  readonly #resetHtmlInspectEffect = effect(() => {
    this.previewKind()
    this.content()
    this.filePath()
    this.fileName()
  ...
  readonly #htmlPreviewUrlEffect = effect((onCleanup) => {
    ...
    const objectUrl =
      previewKind === 'html' && typeof content === 'string' && (!url || inspectAvailable)
        ? createHtmlPreviewObjectUrl(token ? buildHtmlInspectorPreviewContent(content, token) : content)
        : null

    this.htmlPreviewUrl.set(previewKind === 'html' ? objectUrl || url : null)
  ...
  private syncHtmlInspectMode() {
    ...
    frameWindow.postMessage(
      {
        enabled: this.htmlInspectAvailable() && this.htmlInspectMode(),
