...
export class PluginAccountBindingPermissionService implements AccountBindingPermissionService {
	private readonly logger = new Logger(PluginAccountBindingPermissionService.name)
	...
	async resolveBoundUser<TUser = any>(input: ResolveBoundUserInput): Promise<TUser | null> {
		...
		this.logger.log(
			`[sso-debug] permission resolveBoundUser provider=${input?.provider ?? 'null'} subjectId=${input?.subjectId ?? 'null'} inputTenantId=${input?.tenantId ?? 'null'} contextTenantId=${contextTenantId ?? 'null'} effectiveTenantId=${tenantId ?? 'null'}`
		)

		if (!tenantId) {
			this.logger.warn('[sso-debug] permission resolveBoundUser skipped because effectiveTenantId is null')
			return null
		}
		...
		this.logger.log(
			`[sso-debug] permission resolveBoundUser result provider=${input?.provider ?? 'null'} subjectId=${input?.subjectId ?? 'null'} userId=${(user as { id?: string } | null)?.id ?? 'null'}`
		)
