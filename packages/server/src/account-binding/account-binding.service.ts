...
export class AccountBindingService {
	...
	async resolveUser(input: ResolveUserInput): Promise<User | null> {
		...
		this.logger.log(
			`[sso-debug] accountBinding.resolveUser start tenantId=${tenantId} provider=${provider} subjectId=${subjectId}`
		)
		...
		this.logger.log(
			`[sso-debug] accountBinding.resolveUser bindingLookup bindingId=${binding?.id ?? 'null'} bindingUserId=${binding?.userId ?? 'null'}`
		)
		...
		if (binding?.userId) {
			...
			this.logger.log(
				`[sso-debug] accountBinding.resolveUser boundUserLookup userId=${boundUser?.id ?? 'null'} hasHash=${boundUser?.hash ? 'true' : 'false'} ghost=${boundUser ? String(this.isGhostUser(boundUser)) : 'n/a'} thirdPartyId=${boundUser?.thirdPartyId ?? 'null'}`
			)

			if (boundUser && !this.isGhostUser(boundUser)) {
				this.logger.log(`[sso-debug] accountBinding.resolveUser hit binding userId=${boundUser.id}`)
				return boundUser
			}
		}

		this.logger.warn(
			`[sso-debug] accountBinding.resolveUser returning null tenantId=${tenantId} provider=${provider} subjectId=${subjectId}`
		)
		return null
	}
