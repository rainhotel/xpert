...
export class XpertPublishHandler implements ICommandHandler<XpertPublishCommand> {
    ...
    async publish(xpert: Xpert, version: string, draft: TXpertTeamDraft) {
        ...
        if (draft.nodes) {
            const newAgents = []
            const agentNodes = draft.nodes.filter((node) => node.type === 'agent') as (TXpertTeamNode & {
                type: 'agent'
            })[]
            // All relative experts
            const xpertNodes = draft.nodes.filter((node) => node.type === 'xpert') as (TXpertTeamNode & {
                type: 'xpert'
            })[]
            ...
            for await (const node of agentNodes) {
                // Collect toolsetIds
                const toolsetIds = draft.connections
                    .filter((_) => _.type === 'toolset' && _.from === node.key)
                    .map((_) => _.to)
                const knowledgebaseIds = draft.connections
                    .filter((_) => _.type === 'knowledge' && _.from === node.key)
                    .map((_) => _.to)
                totalToolsetIds.push(...toolsetIds)
                totalKnowledgebaseIds.push(...knowledgebaseIds)

                // All relative experts
                const xpertIds = draft.connections
                    .filter((_) => _.type === 'xpert' && _.from === node.key)
                    .map((_) => _.to)
                const collaboratorNames = xpertIds
                    .map((id) => xpertNodes.find((_) => _.key === id)?.entity.name)
                    .filter(Boolean)
            ...
            draft.nodes
                .filter((node) => node.type === 'workflow' && node.entity.type === WorkflowNodeTypeEnum.KNOWLEDGE)
                .forEach((node) => {
